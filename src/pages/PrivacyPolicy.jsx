...
const PrivacyPolicy = () => {
  const sections = [
    {
      icon: <Eye size={24} className="text-black" />,
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 font-sans">
        {/* Sidebar Navigation */}
        <div className="lg:col-span-1 hidden lg:block sticky top-28 self-start">
          <div className="bg-white rounded-none p-6 border border-slate-200 shadow-sm">
            <h3 className="font-bold text-gray-800 mb-4 text-xs uppercase tracking-wider">Quick Navigation</h3>
            <nav className="space-y-3">
              {sections.map((section, idx) => (
                <a 
                  key={idx} 
                  href={`#section-${idx}`} 
                  className="block text-sm text-slate-500 hover:text-black hover:font-bold hover:translate-x-1 transition-all duration-200"
                >
                  {section.title}
                </a>
              ))}
        ...
        <div className="lg:col-span-3 space-y-8">
          <div className="bg-white rounded-none p-8 md:p-10 border border-slate-200 shadow-sm leading-relaxed text-slate-800">
            <p className="text-xs text-slate-400 mb-8 font-bold uppercase tracking-wider">Last Updated: May 19, 2026</p>
            
            <p className="mb-6 text-lg font-medium leading-relaxed">
              At Kelvornex, we are committed to protecting the privacy of our learners and users. This Privacy Policy details our practices regarding the collection, use, and disclosure of information we receive through our platform, online online courses, and associated web services.
            </p>
            ...
            <div className="space-y-12">
              {sections.map((sec, idx) => (
                <div key={idx} id={`section-${idx}`} className="scroll-mt-28 space-y-4">
                  ...
                  <p className="text-slate-600 leading-relaxed text-sm md:text-base font-medium">{sec.content}</p>
                </div>
              ))}
            </div>

            {/* Support section */}
            <div className="mt-12 p-6 rounded-none bg-slate-50 border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
                <h4 className="font-bold text-gray-900 text-lg mb-1">Have questions about our privacy practices?</h4>
                <p className="text-sm text-slate-500 font-semibold">Our dedicated Data Protection team is ready to help you with your inquiry.</p>
              </div>
              <a 
                href="mailto:privacy@kelvornex.com" 
                className="bg-black hover:bg-transparent border-2 border-black text-white hover:text-black font-bold px-6 py-3.5 rounded-full transition-all text-xs tracking-wider uppercase shadow-md shadow-black/10 shrink-0 cursor-pointer"
              >
                Contact Data Team
              </a>
            </div>
